...
CENTER_FREQ_HZ = 2.409975586e9    # Hendrix CW frequency
SPAN_HZ        = 10_000             # 10 kHz span
RBW_HZ         = 1_000              # 1 kHz RBW
VBW_HZ         = 1_000              # 1 kHz VBW
...
class BB60Spike:
    ...
    def assert_measurement_state(self):
        """
        Reassert full measurement state.
        MUST be called every cycle because GUI can silently change things.
        """
...
def new_run_folder():
    ...
    path = os.path.join(BASE_LOG_DIR, ts)
    ensure_dir(path)
    return path
...
def open_shift_csv(folder, shift_idx):
    ...
    w = csv.writer(f)
    w.writerow(["timestamp_iso", "frequency_hz", "amplitude_dbm"])
    f.flush()
...
def main():
    print("[CW LOGGER] Starting CW frequency/amplitude logger")
