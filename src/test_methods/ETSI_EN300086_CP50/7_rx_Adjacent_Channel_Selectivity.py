...
def run(params, radio_ctrl, equip_mgr, test_results):
    ...
    for wanted_test_freq_MHz in wanted_freqs:
        for bandwidth in bandwidths:
            if bandwidth == "NB":unwanted_freq_offsets=[-12.5,12.5]
            if bandwidth == "WB":unwanted_freq_offsets=[-25,25]
    ...
    # 8.c) Plot ACS: SINAD vs Offset for each RX level, separately for NB and WB
    for bw in df['bandwidth'].unique():
        plt.figure()
        for level in sorted(df.loc[df['bandwidth'] == bw, 'wanted_rx_level_dBm'].unique()):
            subset = df[(df['bandwidth'] == bw) & (df['wanted_rx_level_dBm'] == level)]
            # offset is in kHz
            plt.plot(subset['offset'], subset['average_sinad_dB'], marker='o', label=f"{level:.1f} dBm")

        plt.xlabel('Offset (kHz)')
        plt.ylabel('SINAD (dB)')
        plt.title(f"{this_script_abr} {bw}: SINAD vs Offset")
        plt.legend(title='RX Level')
        plt.grid(True)

        png_path = os.path.join(out_dir, f"{log_script_prefix}_acs_plot_{bw}.png")
        plt.savefig(png_path, dpi=200)
        plt.close()
        print_green(f"[{this_script_abr}] {bw} plot saved to {png_path}")
